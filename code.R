...
# violin plot for Revenue and ProductRelated
ggplot(data, aes(x = ProductRelated,y=Revenue, fill = Revenue))+geom_violin(trim = FALSE)
#plot for administrative pages
ggplot(data, aes(x =Administrative,fill=Revenue))+geom_bar()
#plot for informational pages
ggplot(data, aes(x =Informational,fill=Revenue))+geom_bar()
#plot for Operating System type
ggplot(data, aes(x =OperatingSystems,fill=Revenue))+geom_bar()
#histogram for Bounce Rate
hist(data$BounceRates,col="blue")
#histogram for Exit Rates
hist(data$ExitRates,col="pink")
#histogram for page values
hist(data$PageValues,col="darkmagenta")


# set seed 
set.seed(12345678)

# saved prepossessed data
